...
#if defined (CONFIG_MACH_LGE_FX3_VZW) || defined (CONFIG_MACH_LGE_F6_TMUS) || defined(CONFIG_MACH_LGE_FX3Q_TMUS) 
		if(pseudo_batt_info.mode == 1)
...
static void set_vzw_charging_state(struct pm8921_chg_chip *chip)
{
	int usb_ma;
	...
	if (usb_ma > 0 && usb_ma < VZW_CHG_MIN_CURRENT) {
		chg_state = VZW_NOT_CHARGING;
		// disable charging ??
	}
	// incompatible charger, but possible to charge in under current
	else if (usb_ma >= VZW_CHG_MIN_CURRENT && usb_ma < VZW_CHG_RATED_CURRENT) {
		chg_state = VZW_UNDER_CURRENT_CHARGING;
	}
	// compatible charger, possible to charge in rated current
	else if (usb_ma >= VZW_CHG_RATED_CURRENT) {
		chg_state = VZW_NORMAL_CHARGING;
	}
	else {
		chg_state = VZW_NOT_CHARGING;
		pr_err("[AICL] Can's set VZW_CHG_STATE, usb_ma = %d \n", usb_ma);
		return;
	}
...
*/
		...
		if (usb_ma <= 100) {
#ifdef CONFIG_LGE_PM_VZW_FAST_CHG
			if(usb_target_ma == 100 && !is_finished_chg_state_check) {
				pr_info("[AICL] usb_target_ma == 100 && !is_finished_chg_state_check \n");
				set_vzw_charging_state(chip);
...
#ifdef CONFIG_LGE_PM_VZW_FAST_CHG
	if(!is_finished_chg_state_check && !charging_disabled && chip->aicl && usb_target_ma > 0) {
		fsm_state = pm_chg_get_fsm_state(chip);
...
#ifdef CONFIG_LGE_PM_TRKLCHG_IN_KERNEL
		m = 2;	// set default current to 500mA
#else
		m = 1;
#endif
/*                                                */
