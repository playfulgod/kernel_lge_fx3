...
static void melfas_ts_restore_irq(struct work_struct *work){
	...
	if(gpio_get_value(11) == 0){
		...
		printk(KERN_DEBUG "[Touch] Touch inetrrupt pin is low. before = %d, current = %d\n", touch_before_irq_cnt, touch_irq_cnt);
...
#ifdef DEBUG_TOUCH_IRQ
	schedule_delayed_work(&ts->irq_restore_work, msecs_to_jiffies(HZ * 100)); // 3 sec
#endif
...
#ifdef DEBUG_TOUCH_IRQ
	flush_delayed_work_sync(&ts->irq_restore_work); 
#endif
...
#endif
	if(irq_flag == 1){
		printk(KERN_INFO "[Touch] ENABLE_IRQ \n");
		enable_irq(ts->client->irq);
		irq_flag = 0;
	}
